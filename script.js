...
function showPost(postUrl, postTitle) {
    ...
    fetch(postUrl).then(response => response.text()).then(htmlString => {
        const parser = new DOMParser();
        const doc = parser.parseFromString(htmlString, 'text/html');
        const postContentHTML = doc.body.innerHTML;
        const postViewerHTML = `<div class="window-box post-viewer-window"><div class="window-header"><span class="window-title">${postTitle}</span><div class="window-buttons"><i class="fa-solid fa-xmark" onclick="showHomePage()"></i></div></div><div class="window-content post-viewer-content">${postContentHTML}</div></div>`;
        windowsArea.innerHTML = postViewerHTML;
    }).catch(error => { console.error('포스트 로딩 오류:', error); windowsArea.innerHTML = `<p>포스트를 불러올 수 없습니다.</p>`; });
}

function loadPosts() {
    const postContainer = document.getElementById('post-list-container');
    if (!postContainer) return;
    fetch('posts.json').then(response => response.json()).then(posts => {
        let htmlContent = '';
        posts.forEach(post => {
    ...
    }).catch(error => { console.error('포스트 목록 로딩 오류:', error); postContainer.innerHTML = '<p>포스트를 불러올 수 없습니다.</p>'; });
}
...
function showHomePage() {
    ...
    runDday(); runCalendar(); runWindowFocus(); loadPosts();
}
...
function loadGalleryImages() {
    ...
    fetch('gallery.json').then(response => response.json()).then(images => { galleryImages = images; showGalleryGrid(); })
...
function showGalleryGrid() {
    ...
    galleryContentWrapper.querySelector('.gallery-grid-container').addEventListener('click', (e) => {
        if (e.target.tagName === 'IMG') initImageViewer(parseInt(e.target.dataset.index, 10));
    });
}
...
function updateImageViewer(index) {
    currentIndex = index;
    const imageEl = document.getElementById('viewer-image-element'), prevBtn = document.getElementById('viewer-prev-btn'), nextBtn = document.getElementById('viewer-next-btn');
...
function navigateGallery(direction) {
    ...
    if (newIndex >= 0 && newIndex < galleryImages.length) updateImageViewer(newIndex);
}
...
function showCharacterPage() {
    ...
    const characterWindowsHTML = `
        <div class="window-box character-window" id="leona-window">
            ...
            <div class="window-content"><img src="leona-profile.jpg" class="character-profile-img" id="leona-profile-img"></div>
        </div>
        <div class="window-box character-window" id="nea-window">
            ...
            <div class="window-content"><img src="nea-profile.jpg" class="character-profile-img"></div>
        </div>
    `;
    ...
    const setWindowHeights = () => {
        ...
        if(imgWidth > 0) {
            const newContentHeight = (imgHeight / imgWidth) * fixedWidth;
            const newWindowHeight = newContentHeight + 28;
            leonaWindow.style.height = `${newWindowHeight}px`;
            neaWindow.style.height = `${newWindowHeight}px`;
        }
    };

    if (leonaImg.complete && leonaImg.naturalWidth > 0) {
        setWindowHeights();
    } else {
